...
export const fileListingHtml = (entries, dirPath) => {
  ...
  let html = '<div class="fv-files-header"><span>📁 ' + entries.length + ' アイテム</span>'
    + '<div class="fv-actions"><button class="fv-btn" onclick="window.createFolder()">+ フォルダ</button></div></div>';
  ...
  for (const e of sorted) {
    ...
    html += '</a>';
    html += '<button class="fv-del-btn" onclick="event.stopPropagation();window.deleteEntry(\'' + esc(e.path) + '\')" title="削除">✕</button>';
    html += '</div>';
  }
  html += '</div>';
  return html;
...
export const bookmarkItemHtml = (bm, i, folderPath, mode) => {
  ...
  if (mode === 'list') {
    return '<div class="fv-bookmark-item">'
      ...
      + '</a>'
      + '<button class="fv-edit-btn" onclick="window.editBookmark(' + i + ')" title="編集">✎</button>'
      + '<button class="fv-del-btn" onclick="window.deleteBookmark(' + i + ')" title="削除">✕</button>'
      + '</div>';
  }
  // thumbnail card mode
...
export const folderViewHtml = (folderPath, entries, bookmarks, mode, hasIndex) => {
  ...
  if (mode === 'list') {
    html += '<div class="fv-bookmarks-header"><span>🔖 Bookmarks</span>'
      + '<div class="fv-actions"><button class="fv-btn" onclick="window.showBookmarkAddDialog(\'\')">+ 追加</button></div></div>';
    if (!bookmarks || !bookmarks.length) {
      html += '<div class="fv-empty">📥 URLをドロップしてブックマーク追加</div>';
    } else {
